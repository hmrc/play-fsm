...
trait JourneyController[RequestContext] {
  ...
  /**
    ...
    */
  final object actions extends SimpleActionsDSL {
    ...
    final def whenAuthorised(
      ...
      new WhenAuthorised(withAuthorised)
    ...
    ): WhenAuthorisedWithRetrievals[User] =
      new WhenAuthorisedWithRetrievals[User](withAuthorised)
  }
  ...
  protected sealed abstract class SimpleActionsDSL extends ActionsDSL {
    ...
    final object SimpleResolver extends Resolver {
      ...
      override def apply[P](f: TransitionWithRequestAnd[P], r: Request[_]): P => model.Transition =
        f(r)
  ...
  protected sealed abstract class DoubleArgumentActionsDSL[A, B](outer: SingleArgumentActionsDSL[A])
      extends ActionsDSL {
  ...
  /**
    * Actions DSL: Action builder wrapping async function returning some value.
    */
  final class GetAsync1[A, T](get: Request[_] => A => Future[T], outer: SingleArgumentActionsDSL[A])
  ...
  @deprecated("Prefer DSL actions.whenAuthorised[User].bindForm(form).apply(transition)", "0.72.0")
